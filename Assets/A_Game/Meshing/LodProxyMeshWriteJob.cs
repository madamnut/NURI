...
/// <summary>
/// Writes a single render-only proxy mesh for a world-aligned multi-chunk LOD region.
/// </summary>
[BurstCompile]
public struct LodProxyMeshWriteJob : IJobParallelFor
{
    ...
    public void Execute(int index)
    {
        ...
        byte d0 = TerrainDensityUtility.SampleDensity(Settings, worldBaseX, localY + 0, worldBaseZ);
        byte d1 = TerrainDensityUtility.SampleDensity(Settings, worldBaseX + HorizontalStep, localY + 0, worldBaseZ);
        byte d2 = TerrainDensityUtility.SampleDensity(Settings, worldBaseX + HorizontalStep, localY + 0, worldBaseZ + HorizontalStep);
        byte d3 = TerrainDensityUtility.SampleDensity(Settings, worldBaseX, localY + 0, worldBaseZ + HorizontalStep);
        byte d4 = TerrainDensityUtility.SampleDensity(Settings, worldBaseX, localY + 1, worldBaseZ);
        byte d5 = TerrainDensityUtility.SampleDensity(Settings, worldBaseX + HorizontalStep, localY + 1, worldBaseZ);
        byte d6 = TerrainDensityUtility.SampleDensity(Settings, worldBaseX + HorizontalStep, localY + 1, worldBaseZ + HorizontalStep);
        byte d7 = TerrainDensityUtility.SampleDensity(Settings, worldBaseX, localY + 1, worldBaseZ + HorizontalStep);
        ...
        for (int i = 0; i < MarchingCubesTables.TriangleTableStride; i += 3)
        {
            ...
            MaterialInfo[vertexStart + 0] = BuildBlendInfoForEdge(e0, worldBaseX, localY, worldBaseZ);
            MaterialInfo[vertexStart + 1] = BuildBlendInfoForEdge(e1, worldBaseX, localY, worldBaseZ);
            MaterialInfo[vertexStart + 2] = BuildBlendInfoForEdge(e2, worldBaseX, localY, worldBaseZ);
