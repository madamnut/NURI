...
public sealed class ChunkData : IDisposable
{
    ...
    {
        Coord = coord;
        Density = new NativeArray<sbyte>(WorldConstants.ChunkSampleCount, Allocator.Persistent);
        MaterialIds = new NativeArray<byte>(WorldConstants.ChunkCellCount, Allocator.Persistent);
        MaterialAmounts = new NativeArray<byte>(WorldConstants.ChunkCellCount, Allocator.Persistent);
        DirtySubChunkMask = 0;
    }
    ...
    /// </summary>
    public bool IsCreated => Density.IsCreated && MaterialIds.IsCreated && MaterialAmounts.IsCreated;
    ...
    /// density 샘플 값을 읽는다.
    /// </summary>
    public sbyte GetDensity(int sampleX, int sampleY, int sampleZ)
    {
    ...
    /// density 샘플 값을 기록한다.
    /// </summary>
    public void SetDensity(int sampleX, int sampleY, int sampleZ, sbyte value)
    {
    ...
    public byte GetMaterialAmount(int cellX, int cellY, int cellZ)
    {
        return MaterialAmounts[WorldMath.CellIndex(cellX, cellY, cellZ)];
    }

    public void SetMaterialAmount(int cellX, int cellY, int cellZ, byte value)
    {
        MaterialAmounts[WorldMath.CellIndex(cellX, cellY, cellZ)] = value;
    }
    ...
    public void Dispose()
    {
        ...
        if (MaterialAmounts.IsCreated)
        {
            MaterialAmounts.Dispose();
        }
